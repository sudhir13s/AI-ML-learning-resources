...
![Animated — the same measured run, step by step: max|ΔW| sits at exactly 0 at step 0 (B = 0, so the adapted layer *is* the pretrained layer), then climbs as the adapter learns its way away from the pretrained point. The live version of the figure above; generated by `code/make_animations_12.py` from the same seeded run as `lora_peft.py`.](../images/lora_delta_growth.gif)
...
![Animated — the rank sweep revealed one r at a time. Below the true update rank the adapter underfits (loss stays high); at r = 4 the loss falls off a cliff; past it, params keep rising (green, linear in r) while the fit no longer improves. The live version of the figure above; generated by `code/make_animations_12.py` from the same measured sweep.](../images/lora_rank_vs_fit.gif)
