...
![Animated — the affine map made literal, one value at a time. Each real value x (blue circle) drops and snaps to its nearest tick on the int8 ruler, stored as the integer q (here s = 0.01654, z = 0); dequantizing gives x̂ = s·q (red square), and the short arrow is the quantization error — never more than half a tick. The loudest value (2.10) pins the scale at exactly q = 127. Generated by `tools/make_animations_10.py`.](images/affine_number_line.gif)
