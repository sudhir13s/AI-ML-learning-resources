...
![Animated — softening the teacher to reveal dark knowledge. The same teacher logits, with the distillation temperature swept: at T=1 the 'cat' bar (navy) dwarfs everything and the dog/lynx mass is invisible; as T rises, the dog and lynx mass (amber) lifts into view — that emerging off-target mass IS the dark knowledge the student learns from — until, pushed too far, the distribution over-softens toward flat. Generated by `tools/make_animations_11.py`.](../images/kd_temperature_softening.gif)
