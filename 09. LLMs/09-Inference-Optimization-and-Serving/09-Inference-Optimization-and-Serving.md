...
![Animated — the same 6 ragged requests (4/6/20/60/5/8 tokens) on a 3-slot engine, played forward as a clock sweeps. Static batching (top) runs locked waves: short requests finish early but their slots sit idle (hatched) until the wave's longest request ends — 800 ms. Continuous batching (bottom) backfills a freed slot the very next step (D starts in slot 1 the instant A finishes) and visibly finishes first — 640 ms. Same hardware, same work, 1.25× faster. Generated by `code/make_animations_09.py`.](../images/serving_batching_timeline.gif)
