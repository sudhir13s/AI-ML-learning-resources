...
![Gantt timeline of the same 6 ragged requests (4/6/20/60/5/8 tokens) on a 3-slot engine, modeled. Static batching (top) runs requests in locked waves: short requests A, B, E, F finish early but their slots sit idle (hatched) until the wave's longest request ends — 800 ms makespan, 43% utilization. Continuous batching (bottom) backfills a freed slot the very next step (D starts in slot 1 the instant A finishes) — 640 ms makespan, 54% utilization. Same hardware, same work, 1.25× faster.](../images/serving_batching_timeline.png)
