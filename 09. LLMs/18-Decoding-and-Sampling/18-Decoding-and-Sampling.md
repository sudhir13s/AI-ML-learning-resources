...
![The same peaked next-token distribution under temperature T=0.5, 1.0, 2.0. At T=0.5 nearly all mass sits on the single top token (entropy 0.03 bits — almost greedy); at T=1.0 the model's own distribution (entropy 0.61); at T=2.0 the mass spreads toward uniform (entropy 2.20 bits). Temperature is the dial between "trust the top" and "spread it around."](../images/dec_temperature_softmax.png)
