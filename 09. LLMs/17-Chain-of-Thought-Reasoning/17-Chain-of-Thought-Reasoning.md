...
![Animated — the same problem, two prompt structures. Left: asked for the answer only, the model blurts "9" (a plausible-but-wrong shortcut). Right: with "Let's think step by step" the chain of thought unfolds line by line — start 12, pour out 4 leaves 8, add 5 gives 13 — and it lands the correct answer. Nothing about the model changed; only the prompt did. Generated by `code/make_animations_17.py`.](../images/cot_direct_vs_cot_prompt.gif)
