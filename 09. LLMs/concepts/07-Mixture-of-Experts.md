...
Now recall what the [scaling laws](03-Scaling-Laws.md) tell us: model quality improves predictably as you add **parameters** (and data and compute). So you want *more parameters*. But in a dense model, more parameters means proportionally more FLOPs **per token**, which means proportionally more cost to train *and* to serve — for **every** token, forever. Training a dense 1-trillion-parameter model means running a trillion parameters for every one of trillions of training tokens. The compute bill is astronomical, and most of it is spent running parameters that a given token didn't need.
