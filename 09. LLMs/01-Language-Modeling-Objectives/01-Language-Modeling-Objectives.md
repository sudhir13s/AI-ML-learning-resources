...
![Animated — autoregressive generation, one token at a time. The model emits a token, **appends it to the context**, then conditions on that growing past to predict the next — "see only the past, predict forward," which is exactly the causal apprentice's *finish-my-sentence* exercise. The top row is the sequence built so far (the just-emitted token in green); the bottom panel is the next-token distribution the model samples from each step. Generated by `tools/make_animation_01.py`.](../images/lm_autoregressive.gif)
...
> **Runnable project and a step-by-step notebook:** the same verified code lives as a clean script and an executed teaching notebook next to this page — see the [step-by-step teaching notebook](code/01-Language-Modeling-Objectives.ipynb) and the [runnable demo script](code/language_modeling_objectives.py) (run it with `python language_modeling_objectives.py`). Every figure on this page is reproduced from those same seeded functions by [`tools/make_figures_01.py`](tools/make_figures_01.py), and the autoregressive animation by [`tools/make_animation_01.py`](tools/make_animation_01.py) — so no number in a figure can drift from the prose or the demo.
