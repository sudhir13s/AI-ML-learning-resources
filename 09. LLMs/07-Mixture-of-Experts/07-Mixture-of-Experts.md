...
![Animated — sparse routing, token by token. Each token reaches the router and is dispatched to its top-2 of 8 experts (arrow thickness ∝ gate; the chosen experts light up green), and its output is the gate-weighted combine y = g₁·E_a(x) + g₂·E_b(x). Watch the per-expert load build up across tokens — every token touches only 2 of 8 experts, which is the sparsity (and the load-balancing problem). Generated by `code/make_animations_07.py`.](../images/moe_routing_flow.gif)
