...
chapter: 2
level: advanced
built_from: ["kv-cache", "kv-cache/variants"]
interview_frequency: high
template: concept-chapter
updated: 2026-07-14
tier: flagship
est_minutes: 25
leads_to: ["kv-cache/flashattention-flashdecoding", "kv-cache/production"]
---
...
- blocks are **addressable**, which unlocks sharing (L4) and shipping (disaggregation, [chapter 4](05-KV-Cache.ch4-production.md)).
...
> **Gotcha:** a prefix cache needs **invalidation**. Change the system prompt but keep its cache key and every request silently reuses the wrong KV — the "stale prefix" incident in [chapter 4](05-KV-Cache.ch4-production.md).
...
- **INT8/INT4** go further but need scale-factor machinery — per-channel for K, per-token for V (the KIVI asymmetry, mechanics in [chapter 1](05-KV-Cache.ch1-variants.md)).
...
- **Sliding window + attention sinks** — cap `seq_len` at $w$; keep ~4 sink tokens so softmax has somewhere to put its mandatory probability mass (StreamingLLM). Constant memory on endless streams; genuine forgetting outside the window ([chapter 1](05-KV-Cache.ch1-variants.md)).
...
Next: [Chapter 3 — FlashAttention and FlashDecoding](05-KV-Cache.ch3-flashattention-flashdecoding.md): the ladder decided how many bytes exist; the kernels decide how close to peak bandwidth you move them.
