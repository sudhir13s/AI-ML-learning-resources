...
![Animated — the redundant work piling up. At each decode step the no-cache path re-projects K/V for *all* tokens so far (red) while the cache projects just the one new token (green); the shaded gap is pure waste, and the cumulative total turns an O(n²) climb into an O(n) line. The live version of the figure above; generated by `code/make_animations_05.py`.](../images/kv_recompute_waste.gif)
...
![Animated — the cache climbing with context. For 7B/13B/70B (MHA, batch 1, fp16) the KV cache grows linearly with context length and, by 64K tokens, crosses each model's own weight footprint (dotted lines) — at long context the cache, not the weights, is the memory you run out of. The live version of the figure above; generated by `code/make_animations_05.py`.](../images/kv_memory_growth.gif)
