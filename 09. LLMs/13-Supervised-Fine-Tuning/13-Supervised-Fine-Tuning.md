...
![Animated — the prompt mask switching on. The single most important SFT visual: one (prompt, response) token stream. It starts as if loss were computed on every token (all green), then SFT masks the prompt tokens to -100 one by one (slate) until loss flows ONLY over the response tokens (green) — right of the prompt/response boundary. Generated by `code/make_animations_13.py`.](../images/sft_prompt_mask.gif)
