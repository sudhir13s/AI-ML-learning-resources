...
> **Note:** this also tells you *when SFT will disappoint you*. If the task needs **knowledge** the base model lacks (a private codebase, post-cutoff facts, a niche domain), SFT on a handful of examples won't conjure it — you need either **continued pretraining** on that domain, **retrieval** ([RAG Fundamentals](../../15.%20RAG_and_LLM_Applications/01-RAG-Fundamentals/01-RAG-Fundamentals.md)) to supply the facts at inference, or far more data. SFT reshapes behavior; it is a poor tool for installing missing knowledge.
