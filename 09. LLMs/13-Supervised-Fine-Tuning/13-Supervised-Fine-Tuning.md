...
> **Note:** this also tells you *when SFT will disappoint you*. If the task needs **knowledge** the base model lacks (a private codebase, post-cutoff facts, a niche domain), SFT on a handful of examples won't conjure it — you need either **continued pretraining** on that domain, **retrieval** ([RAG Fundamentals](../../llms-applications-and-agents/rag-and-knowledge-systems/rag-foundations/rag-foundations.md)) to supply the facts at inference, or far more data. SFT reshapes behavior; it is a poor tool for installing missing knowledge.
