...
> **Note:** this is the exact same ELBO / "maximize a lower bound because the true objective is intractable" move that powers the [variational autoencoder](../../10.%20GenAI/01-Variational-Autoencoders-VAE-ELBO/01-Variational-Autoencoders-VAE-ELBO.md) and, in spirit, the EM algorithm for [GMMs](../../04.%20Unsupervised_Learning/04-Gaussian-Mixture-Models-and-EM/04-Gaussian-Mixture-Models-and-EM.md). Variational EM for LDA *is* EM, with an approximate E-step because the exact posterior over $(\theta, \mathbf{z})$ doesn't factor nicely. If you've seen the ELBO once, you've seen it everywhere in this corner of ML.
