...
This is the link to [n-gram language models](../04-N-gram-Language-Models-and-Smoothing/04-N-gram-Language-Models-and-Smoothing.md): the same contiguous-window idea, used there for *prediction* and here for *features*. In practice `TfidfVectorizer(ngram_range=(1,2))` mixes unigrams and bigrams, and bigrams reliably lift text-classification accuracy a few points — capturing negation, fixed phrases (`new york`, `machine learning`), and short idioms.
...
> **Runnable script and a step-by-step notebook:** the verified code lives as a clean source-of-truth script and an executed teaching notebook next to this page — see the [step-by-step teaching notebook](code/03-Bag-of-Words-and-TF-IDF.ipynb) and the [runnable demo script](code/bow_tfidf.py) (run it with `python bow_tfidf.py`). Every number on this page and every figure is produced by importing from that one script, so they cannot drift. The notebook proves each qualitative claim with an `assert` before it prints the number.
...
- **No semantics (synonymy).** Distinct words are orthogonal axes; `car` and `automobile` have similarity 0. → [Word embeddings](../05-Word-Embeddings-Word2Vec-GloVe-FastText/05-Word-Embeddings-Word2Vec-GloVe-FastText.md) place related words *near* each other so similar meaning ⇒ similar vector.
...
- **No polysemy resolution.** One vector per word type, all senses merged. → [Contextual embeddings](../06-Contextual-Embeddings-ELMo-BERT/06-Contextual-Embeddings-ELMo-BERT.md) (ELMo, BERT) give a *different* vector per occurrence based on context.
- **Out-of-vocabulary words.** Unseen terms have no column and vanish. → Subword [tokenization](../02-Tokenization-and-Subword-Algorithms/02-Tokenization-and-Subword-Algorithms.md) (BPE, WordPiece) and FastText's character n-grams handle novel words.
