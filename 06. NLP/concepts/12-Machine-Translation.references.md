...
2. **See attention fix MT** — read ⭐ [Visualizing A Neural Machine Translation Model](https://jalammar.github.io/visualizing-neural-machine-translation-mechanics-of-seq2seq-models-with-attention/) (**Jay Alammar**), then watch [Stanford CS224N Lec 10 — NMT & attention](https://www.youtube.com/watch?v=IxQtK2SjWWM). *Watch the single-vector bottleneck appear and attention remove it.*
3. **Get the math** — read [SLP3 Ch. 13 "Machine Translation"](https://web.stanford.edu/~jurafsky/slp3/13.pdf) (**Jurafsky & Martin**). *Encoder–decoder MT, beam search, and BLEU, rigorously and free.*
4. **Read the sources** — [Bahdanau attention](https://arxiv.org/abs/1409.0473) → [GNMT](https://arxiv.org/abs/1609.08144) → [Attention Is All You Need](https://arxiv.org/abs/1706.03762). *RNN attention → production NMT → the Transformer, which was introduced for translation.*
5. **Make it concrete** — code it with the [TensorFlow NMT-with-attention tutorial](https://www.tensorflow.org/text/tutorials/nmt_with_attention) or run [opus-mt-fr-en](https://huggingface.co/Helsinki-NLP/opus-mt-fr-en) (the model on the concept page). *Train or run a real translator.*
...
- [Stanford CS224N Lec 10 — Neural Machine Translation & Models with Attention](https://www.youtube.com/watch?v=IxQtK2SjWWM) — **Stanford (Chris Manning)** — the rigorous NMT + attention lecture.
...
- [sacreBLEU](https://github.com/mjpost/sacrebleu) — **Matt Post** — the standard, tokenization-fixed BLEU/chrF/TER implementation; read the README on *why* raw BLEU is not comparable.

**Key papers**:
- [The Mathematics of Statistical Machine Translation: Parameter Estimation (IBM Models)](https://aclanthology.org/J93-2003/) — **Brown et al. (1993)** — the noisy-channel model, word alignment, and IBM Models 1–5 via EM.
...
- [Google's Neural Machine Translation (GNMT)](https://arxiv.org/abs/1609.08144) — **Wu et al. (2016)** — production-scale NMT, WordPiece, and the ~60% error reduction over phrase-based SMT.
...
- [BLEU: a Method for Automatic Evaluation of Machine Translation](https://aclanthology.org/P02-1040/) — **Papineni et al. (2002)** — the n-gram-overlap metric that defined MT evaluation for 20 years.
- [COMET: A Neural Framework for MT Evaluation](https://aclanthology.org/2020.emnlp-main.213/) — **Rei et al. (2020)** — the learned, meaning-aware metric that credits valid paraphrases BLEU misses.
...
- The architecture MT runs on: [08 Sequence-to-Sequence & Encoder–Decoder](08-Sequence-to-Sequence-and-Encoder-Decoder.md)
- Foundations (the *why* behind attention / the Transformer): [Attention Mechanism](../../05.%20Deep_Learning/concepts/15-Attention-Mechanism.md) · [Transformer Architecture](../../05.%20Deep_Learning/concepts/16-Transformer-Architecture.md)
- Supporting techniques: [02 Tokenization & Subword Algorithms](02-Tokenization-and-Subword-Algorithms.md) (BPE) · [17 Decoding Strategies](17-Decoding-Strategies.md) (beam search) · [18 NLP Evaluation Metrics](18-NLP-Evaluation-Metrics.md) (BLEU)
- Related tasks: [13 Text Summarization](13-Text-Summarization.md) · [14 Coreference Resolution](14-Coreference-Resolution.md) (pronoun/gender resolution in document-level MT)
