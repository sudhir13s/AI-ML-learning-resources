...
Every classical topic model operates on the **bag-of-words** representation you met in [Bag-of-Words & TF-IDF](../bag-of-words-and-tf-idf/bag-of-words-and-tf-idf.md): a $D \times V$ matrix where row $d$ is document $d$ and entry $(d, w)$ counts (or TF-IDF-weights) how often word $w$ appears in $d$. Word **order is thrown away** — only counts remain. Schematically, with three tiny documents and a vocabulary of five words:
...
> **Note:** this is the exact same ELBO / "maximize a lower bound because the true objective is intractable" move that powers the [variational autoencoder](../../generative-models/variational-autoencoders-vae-elbo/variational-autoencoders-vae-elbo.md) and, in spirit, the EM algorithm for [GMMs](../../../core-machine-learning/unsupervised-learning/clustering/gaussian-mixture-models-and-em/gaussian-mixture-models-and-em.md). Variational EM for LDA *is* EM, with an approximate E-step because the exact posterior over $(\theta, \mathbf{z})$ doesn't factor nicely. If you've seen the ELBO once, you've seen it everywhere in this corner of ML.
...
> **Tip:** the $+\alpha$ and $+\eta$ in the numerators are **smoothing** straight out of the Dirichlet prior — they're pseudo-counts that stop any probability from ever hitting exactly zero, so a topic can still occasionally emit a word it hasn't emitted yet. The same Laplace-style smoothing you saw in [n-gram models](../n-gram-language-models-and-smoothing/n-gram-language-models-and-smoothing.md), arriving here for free from conjugacy.
...
Classical topic models inherit bag-of-words' original sin: **no word order, no context, no synonymy.** To them *bank* (river) and *bank* (money) are one word, and *car* and *automobile* are unrelated. The modern fix replaces counts with **contextual embeddings** ([Sentence & Document Embeddings](../sentence-and-document-embeddings/sentence-and-document-embeddings.md)) and clusters *those*.
...
> **Tip:** before you reach for a topic model at all, ask whether you actually have a **labeling problem** in disguise. If you already know the categories you want, a supervised [text classifier](../text-classification-and-sentiment-analysis/text-classification-and-sentiment-analysis.md) will beat any topic model at sorting documents into them. Topic modeling shines precisely when you *don't* know the categories yet — its job is discovery, not classification.
