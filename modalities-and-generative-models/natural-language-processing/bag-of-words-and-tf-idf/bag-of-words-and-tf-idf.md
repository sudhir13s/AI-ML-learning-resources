...
This is the link to [n-gram language models](../n-gram-language-models-and-smoothing/n-gram-language-models-and-smoothing.md): the same contiguous-window idea, used there for *prediction* and here for *features*. In practice `TfidfVectorizer(ngram_range=(1,2))` mixes unigrams and bigrams, and bigrams reliably lift text-classification accuracy a few points — capturing negation, fixed phrases (`new york`, `machine learning`), and short idioms.
...
- **No semantics (synonymy).** Distinct words are orthogonal axes; `car` and `automobile` have similarity 0. → [Word embeddings](../word-embeddings-word2vec-glove-fasttext/word-embeddings-word2vec-glove-fasttext.md) place related words *near* each other so similar meaning ⇒ similar vector.
...
- **No polysemy resolution.** One vector per word type, all senses merged. → [Contextual embeddings](../contextual-embeddings-elmo-bert/contextual-embeddings-elmo-bert.md) (ELMo, BERT) give a *different* vector per occurrence based on context.
- **Out-of-vocabulary words.** Unseen terms have no column and vanish. → Subword [tokenization](../tokenization-and-subword-algorithms/tokenization-and-subword-algorithms.md) (BPE, WordPiece) and FastText's character n-grams handle novel words.
