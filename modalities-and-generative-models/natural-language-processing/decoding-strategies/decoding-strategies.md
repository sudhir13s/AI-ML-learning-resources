...
The idea exploits the fact that autoregressive **decode is memory-bandwidth-bound** (see [09 LLMs · KV Cache](../../../llms-applications-and-agents/inference-and-runtime/kv-cache/kv-cache.md)): each step the big model mostly waits on memory, doing little compute, so verifying *several* tokens in one forward pass costs barely more than generating one. So:
