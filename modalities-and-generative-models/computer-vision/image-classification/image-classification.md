...
- recap **softmax + cross-entropy** and define **top-1 / top-5 accuracy** precisely (deeper math in [Foundations 23](../../../foundations/mathematical-foundations/cross-entropy-and-kl-divergence/cross-entropy-and-kl-divergence.md));
...
> **Note:** this page is the *workflow*. For the convolution operation itself — filters, weight sharing, receptive fields, the LeNet→ResNet lineage — see the sibling [CNNs & Convolution](../../../deep-learning/neural-architectures/cnns-and-convolution/cnns-and-convolution.md). For the two moves that carry this chapter, see [Transfer Learning for Vision](../transfer-learning-for-vision/transfer-learning-for-vision.md) and [Data Augmentation](../data-augmentation/data-augmentation.md). We point to them rather than repeat them.
...
- **Data** — real labeled images, split into train / validation / test with strict discipline (no image and no near-duplicate crosses the split boundary — see [Data Leakage](../../../foundations/data-preparation/data-leakage/data-leakage.md)).
...
A single accuracy number is an *average*, and averages hide structure. Two failure modes matter. First, **class imbalance**: if 95% of your images are class A, a model that always predicts A scores 95% accuracy while being useless — which is why imbalanced problems demand precision/recall, balanced accuracy, or per-class metrics instead (see [Imbalanced Data](../../../foundations/data-preparation/imbalanced-data/imbalanced-data.md) and the classification-metrics intuition). Second, even on a *balanced* dataset, overall accuracy averages over classes that behave very differently — some easy, some hard. The confusion matrix and per-class accuracy expose exactly which classes fail and what they're confused *with*, information the headline number destroys. This is the on-ramp to the specialized metrics that dominate downstream vision tasks — mAP and IoU for [detection and segmentation](../detection-and-segmentation-metrics/detection-and-segmentation-metrics.md), where a single "accuracy" makes even less sense.
...
- **Classification backbones power everything downstream.** The same pretrained backbone that classifies images becomes the feature extractor for [object detection](../object-detection/object-detection.md) (a detector is a classifier run over regions), [semantic segmentation](../detection-and-segmentation-metrics/detection-and-segmentation-metrics.md) (per-pixel classification), and multimodal models. Classification is the foundation the rest of computer vision is built on.
- **The benchmark that drove the field.** ImageNet (1000 classes, 1.2M images) and its top-1/top-5 metric drove every architecture advance from AlexNet through ResNet to [Vision Transformers](../vision-transformers/vision-transformers.md). Understanding classification is understanding the through-line of modern deep learning for vision.
