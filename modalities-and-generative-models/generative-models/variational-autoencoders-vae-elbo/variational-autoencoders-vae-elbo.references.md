...
**In this platform**:
- Concept page (full explanation): [Variational Autoencoders (VAE · ELBO)](variational-autoencoders-vae-elbo.md)
...
- The deterministic precursor: [05 Deep Learning · 19 Autoencoders](../../../deep-learning/neural-architectures/autoencoders/autoencoders.md) — compresses and reconstructs, but its latent space is a bag of holes; the VAE adds the probabilistic, sampleable latent.
- The same ELBO, fit by EM: [04 Unsupervised Learning · 04 Gaussian Mixture Models & EM](../../../core-machine-learning/unsupervised-learning/clustering/gaussian-mixture-models-and-em/gaussian-mixture-models-and-em.md) — the ELBO / KL-gap identity this page reuses; EM climbs it by a per-datapoint E-step, a VAE by an *amortized* encoder network.
- The closed-form KL: [01 Foundations · 23 Cross-Entropy and KL Divergence](../../../foundations/mathematical-foundations/cross-entropy-and-kl-divergence/cross-entropy-and-kl-divergence.md) — where the two-Gaussian KL (the VAE regulariser) is derived; the VAE specializes it to a unit-Gaussian prior.
- The gradient engine: [05 Deep Learning · 02 Backpropagation and Computational Graphs](../../../deep-learning/neural-network-foundations/backpropagation-and-computational-graphs/backpropagation-and-computational-graphs.md) — the reparameterization trick exists so that ordinary backprop can reach the encoder's μ and log σ².
- The variance contrast: [08 Reinforcement Learning · 09 Policy Gradients (REINFORCE)](../../../core-machine-learning/reinforcement-learning/policy-learning/policy-gradients-reinforce/policy-gradients-reinforce.md) — the *score-function* estimator, the high-variance alternative you must use when you *cannot* reparameterize; the VAE's pathwise estimator is the low-variance counterpart.
- The family contrast: [02 GANs and DCGAN](../gans-and-dcgan/gans-and-dcgan.md) — adversarial, sharp, unstable, no likelihood; the VAE is likelihood-based, stable, and blurry.
- Where it goes next: [05 Diffusion Models (DDPM)](../../diffusion-models/diffusion-models-ddpm/diffusion-models-ddpm.md) — a many-step hierarchical VAE trained by a per-step ELBO; the VAE is its one-step conceptual seed.
- Where the VAE lives in the real world: [07 Latent Diffusion / Stable Diffusion](../../diffusion-models/latent-diffusion-stable-diffusion/latent-diffusion-stable-diffusion.md) — diffusion runs in the compact latent space of a pretrained VAE; encode → diffuse → decode.
