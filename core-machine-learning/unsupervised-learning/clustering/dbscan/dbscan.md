...
> **Tip:** the cheat-sheet for "which clustering algorithm?": **round, balanced blobs and you know `k`** → [k-means](../k-means-clustering/k-means-clustering.md). **Weird shapes + outliers, don't know `k`** → DBSCAN. **Overlapping clusters, want soft/probabilistic membership or elliptical clusters** → [GMM](../gaussian-mixture-models-and-em/gaussian-mixture-models-and-em.md). **Want a full hierarchy / dendrogram or small `n`** → [agglomerative](../hierarchical-clustering/hierarchical-clustering.md). **Weird shapes + *varying* density** → HDBSCAN.
...
- **Anomaly / outlier detection** — the **noise** label *is* an outlier detector: points DBSCAN can't place are your anomalies (network-intrusion, fraud, defect detection). See [Anomaly / Outlier Detection](../../anomaly-detection/anomaly-outlier-detection/anomaly-outlier-detection.md).
