...
*Measured, not illustrative: this is `compare_shape` in [`code/gmm_em.py`](code/gmm_em.py) on the same sheared blobs k-means fails on in [chapter 01](../k-means-clustering/k-means-clustering.md) — GMM ARI 1.00, k-means ARI 0.66. Open [the notebook](code/gaussian-mixture-models-and-em.ipynb) (Step 10) to run it yourself.*
...
> **Note:** the diagonal-covariance GMM is precisely the [Gaussian Naive Bayes](../../../supervised-learning/classification/naive-bayes/naive-bayes.md) density assumption (features independent *within* a component → axis-aligned ellipse), and the tied-full-covariance case is the generative model behind **LDA**. GMMs sit in the same Gaussian family — they just learn the component memberships *unsupervised* via EM instead of being handed labels.
...
- **Anomaly / novelty detection** — fit a GMM to "normal" data and flag points with low $p(x)$ (a common alternative to one-class SVM / isolation forest); see [Anomaly & Outlier Detection](../../anomaly-detection/anomaly-outlier-detection/anomaly-outlier-detection.md).
...
- **Initialization & embeddings** — clustering learned embeddings (after [t-SNE](../../dimensionality-reduction/t-sne/t-sne.md)/[UMAP](../../dimensionality-reduction/umap/umap.md) or an autoencoder) where soft, elliptical clusters beat hard spheres.
...
- **Non-convex / weird shapes** (crescents, rings, intertwined spirals): a GMM tries to wrap ellipses around them and fails. Use **density-based** [DBSCAN](../dbscan/dbscan.md) (follows arbitrary shapes, no $K$ needed) or [spectral clustering](../spectral-clustering/spectral-clustering.md) (clusters by graph connectivity).
...
- **Very high dimensions**: `full` covariance's $d^2$ parameters explode and overfit; either restrict to `diag`/`tied`, reduce dimensionality first ([dimensionality reduction](../../dimensionality-reduction/dimensionality-reduction-overview/dimensionality-reduction-overview.md)/[UMAP](../../dimensionality-reduction/umap/umap.md)), or use a different model.
- **Purely hard labels, speed-critical**: if you don't need soft assignments or a density and clusters are roundish, plain [k-means](../k-means-clustering/k-means-clustering.md) is simpler and faster.
...
- **Non-Gaussian or non-convex clusters** → GMMs assume Gaussian components; for crescents/rings/arbitrary shapes use density-based ([DBSCAN](../dbscan/dbscan.md)) or [spectral clustering](../spectral-clustering/spectral-clustering.md) instead.
