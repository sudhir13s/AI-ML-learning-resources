...
[t-SNE](../t-sne/t-sne.md) solved the *visualization* problem beautifully — model pairwise similarities as probabilities, minimize the KL divergence between high-D and low-D, and out come strikingly clean clusters. But it carries three practical pains that became more acute as datasets grew:
...
1. **Scale / normalize first.** UMAP works on a distance, so the same scaling discipline as [k-NN](../../../supervised-learning/classification/k-nearest-neighbors/k-nearest-neighbors.md) applies — standardize features (or L2-normalize and use cosine for text/embeddings) so one large-range feature can't dominate the neighbor graph.
