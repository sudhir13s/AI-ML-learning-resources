...
> **Note:** this page assumes the [MDP](../../foundations/markov-decision-processes/markov-decision-processes.md) framing (states, actions, transitions, reward, discount $\gamma$, and the return $G_t$) and it is the **policy-based counterpart of value-based [Q-learning](../../value-based-learning/q-learning/q-learning.md)** — read that first for the contrast. The policy gradient is computed by ordinary [backpropagation](../../../../deep-learning/neural-network-foundations/backpropagation-and-computational-graphs/backpropagation-and-computational-graphs.md) through $\log\pi_\theta$; we recap what we need but link out for the full treatment.
...
We build the estimator from the ground up, defining every symbol and connecting each term to the intuition. The MDP machinery is recapped, not re-derived — see [MDPs](../../foundations/markov-decision-processes/markov-decision-processes.md).
