...
Recall the [bias–variance decomposition](../../../model-selection-and-evaluation/bias-variance-tradeoff/bias-variance-tradeoff.md). A single shallow tree — a "stump" or a depth-3 tree — is a **weak learner**: it can only carve the input into a handful of regions, so it **underfits**. It has **high bias, low variance**. A deep tree is the opposite: **low bias, high variance** (it can memorize the training set).

Now here's the key strategic question. If your model has too much **variance**, you average many of them — that's [bagging](../bagging/bagging.md), and it's exactly why [random forests](../random-forests/random-forests.md) average deep trees. But averaging does **nothing** for **bias**: the average of many underfit stumps is still an underfit stump. So how do you *reduce bias*?
...
**Case 2 — log-loss / binomial deviance (binary classification).** Here the model outputs a **log-odds** score $F$, and the probability is $p = \sigma(F) = \frac{1}{1+e^{-F}}$ (the [sigmoid](../../classification/logistic-regression/logistic-regression.md)). The per-example loss is the negative log-likelihood

$$L(y, F) = -\big[\,y\log p + (1-y)\log(1-p)\,\big], \qquad p = \sigma(F).$$

Differentiating w.r.t. the score $F$ (using the standard result $\frac{\partial}{\partial F}[-y\log\sigma(F) - (1-y)\log(1-\sigma(F))] = \sigma(F) - y$, exactly as in [logistic regression](../../classification/logistic-regression/logistic-regression.md)):

$$\frac{\partial L}{\partial F} = \sigma(F) - y = p - y \quad\Longrightarrow\quad r = -\frac{\partial L}{\partial F} = y - p.$$

So for classification **the pseudo-residual is $y - p$** — the gap between the true label (0 or 1) and the predicted probability. Same recipe, different loss: each tree fits $y - p$ instead of $y - F$. (For multiclass it generalizes to $y_k - p_k$ per class via the softmax — the same form you saw in the [softmax + cross-entropy](../../../../deep-learning/optimization-and-training/loss-functions/loss-functions.md) gradient.)
...
5. **Final prediction.** After $M$ rounds, $p = \sigma(F_M(x))$, and you threshold at 0.5 (or wherever your precision/recall tradeoff wants — see [Classification Metrics](../../classification/classification-metrics/classification-metrics.md)).

So a boosted classifier accumulates **log-odds** across trees and squashes the total through the sigmoid at the end — exactly like adding up the contributions of many [logistic-regression](../../classification/logistic-regression/logistic-regression.md) terms, but the terms are trees. For **multiclass**, you run $K$ parallel boosting series (one score $F_k$ per class), turn the scores into probabilities with the **softmax**, and each tree fits the per-class residual $y_{ik} - p_{ik}$ — the multiclass cross-entropy gradient, the same form as a neural net's softmax output layer.
...
It's worth tying every knob back to the [bias–variance tradeoff](../../../model-selection-and-evaluation/bias-variance-tradeoff/bias-variance-tradeoff.md), because that's the lens that makes the hyperparameters stop feeling arbitrary.
...
A useful mental model: a forest is a **committee of experts voting at once** — independent opinions averaged to cancel individual mistakes — while boosting is an **apprenticeship**, where each new learner studies exactly where the current team falls short and specializes in fixing it. Both build on the same [decision tree](../decision-trees/decision-trees.md), and the [bias–variance tradeoff](../../../model-selection-and-evaluation/bias-variance-tradeoff/bias-variance-tradeoff.md) is the single lens that explains why each one is configured the way it is (deep-and-many for forests, shallow-and-shrunk for boosting). If you can hold those two pictures side by side, you understand tree ensembles.
...
The bar chart is the whole tree-ensemble story in one measured picture: the single tree is weak, **bagging** (the [random forest](../random-forests/random-forests.md)) cuts its variance to lift $R^2$ from 0.607 to 0.754, and **boosting** then cuts bias to reach 0.788 (scikit-learn) / 0.797 (real XGBoost) — the honest reason GBDTs are the default for tabular data.
...
**→ [Gradient Boosting — references and further reading](gradient-boosting-xgboost.references.md)**
