...
**In this platform**:
- Concept page (full explanation): [Policy Gradients (REINFORCE)](09-Policy-Gradients-REINFORCE.md)
- Runnable code: [the module `reinforce.py`](code/reinforce.py) · [the step-by-step notebook](code/09-Policy-Gradients-REINFORCE.ipynb) — a from-scratch policy network trained by REINFORCE on real Gymnasium `CartPole-v1`, with the score-function gradient asserted equal to the analytic and finite-difference gradients, the baseline's unbiasedness confirmed, and its variance reduction measured; every number computed, none fabricated.
- The value-based sibling (contrast): [06 Q-Learning](../06-Q-Learning/06-Q-Learning.md) — learns $Q(s,a)$ and acts greedily (value-based, off-policy); this page learns the *policy* $\pi_\theta(a\mid s)$ directly (policy-based, on-policy). The two pillars of model-free control.
- The framework it lives in: [01 Markov Decision Processes](../01-Markov-Decision-Processes/01-Markov-Decision-Processes.md) — states, actions, transitions, reward, and $\gamma$, the language the objective $J(\theta)=\mathbb{E}_\tau[R(\tau)]$ is written in.
- The values it builds on: [02 Bellman Equations](../02-Bellman-Equations/02-Bellman-Equations.md) — the state-value $V(s)$ that becomes the *learned baseline / critic* in the next chapter.
- The gradient engine: [05 Deep Learning · Backpropagation and Computational Graphs](../../05.%20Deep_Learning/02-Backpropagation-and-Computational-Graphs/02-Backpropagation-and-Computational-Graphs.md) — the policy gradient is computed by ordinary backprop through $\log\pi_\theta(a_t\mid s_t)$; `loss.backward()` *is* the policy gradient.
- Where the baseline becomes a critic: [10 Actor-Critic (A2C / A3C)](../10-Actor-Critic-A2C-A3C/10-Actor-Critic-A2C-A3C.md) — replace the mean-return baseline with a learned value function $V_\phi(s)$ and bootstrap it; REINFORCE-with-baseline is the seed of actor-critic.
- The modern successor: [12 Proximal Policy Optimization (PPO)](../12-Proximal-Policy-Optimization-PPO/12-Proximal-Policy-Optimization-PPO.md) — fixes REINFORCE's on-policy sample inefficiency with a clipped surrogate and importance sampling; the algorithm behind RLHF.
- The exploration view: [14 Exploration vs Exploitation](../14-Exploration-vs-Exploitation/14-Exploration-vs-Exploitation.md) — a stochastic policy explores *by construction*; the entropy of $\pi_\theta$ is the exploration knob here.
