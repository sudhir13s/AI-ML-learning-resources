...
In the [ReAct chapter](../reason-and-act/reason-and-act.md) we taught a model to call a tool by writing free text — `Action: calculator[481 * 32 + 19]` — and we parsed that text with a regex. It worked. But watch what a real 1.5-billion-parameter model does when you ask it, ReAct-style, to emit a `TOOL: name(args)` line and nothing else:
