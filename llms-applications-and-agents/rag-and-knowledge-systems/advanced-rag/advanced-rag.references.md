...
> Companion link library for **[Advanced RAG (Parent-Document · RAG-Fusion · Self-RAG)](advanced-rag.md)** (the concept page). External sources *and* internal cross-links, kept separate so it can be reused as a standalone list. Grouped by type, best-first. Every entry is free/open (no paywall) and chosen for depth on *this* topic — decoupling retrieval from generation (parent-document / sentence-window / auto-merging), RAG-Fusion, and self-reflective RAG (Self-RAG, CRAG).
...
**In this platform**:
- Concept page (full explanation): [Advanced RAG (Parent-Document · RAG-Fusion · Self-RAG)](advanced-rag.md)
- Foundations this builds on: [RAG Fundamentals](../rag-foundations/rag-foundations.md) · [Document Chunking (the small-vs-big dilemma this resolves)](../chunking/chunking.md) · [Hybrid Search + RRF](../hybrid-search/hybrid-search.md) · [Query Transformation (RAG-Fusion = multi-query + RRF)](../query-transformation/query-transformation.md)
- Composes with: [Re-ranking with Cross-Encoders (order the retrieved pool)](../reranking/reranking.md) · [Agentic RAG (the loop taken further)](../agentic-rag/agentic-rag.md)
- Measure it: [RAG Evaluation (grounding, faithfulness, recall@k, MRR)](../rag-evaluation/rag-evaluation.md) · [Guardrails & Hallucination Mitigation (the support-check, productionized)](../../reasoning-evaluation-and-alignment/hallucination-and-grounding/hallucination-and-grounding.md)
