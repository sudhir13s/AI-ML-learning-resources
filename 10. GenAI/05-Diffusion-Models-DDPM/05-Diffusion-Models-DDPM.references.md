...
5. **Make it concrete** — run this chapter's [notebook](code/05-Diffusion-Models-DDPM.ipynb): a from-scratch DDPM on a real 2-D distribution, with the closed-form forward asserted equal to the iterative chain, and the trained model's samples asserted to match the target by energy distance.
...
**In this platform**:
- Concept page (full explanation): [Diffusion Models (DDPM)](05-Diffusion-Models-DDPM.md)
- Runnable code: [the module `ddpm.py`](code/ddpm.py) · [the step-by-step notebook](code/05-Diffusion-Models-DDPM.ipynb) — a from-scratch DDPM on a real 2-D distribution, with the closed-form forward asserted equal to the iterative chain (the "nice property"), the trained model's samples asserted to match the target by energy distance, and every figure (forward diffusion, noise schedule, reverse trajectory, generation overlay) from the same measured run; every number computed, none fabricated.
- The one-step seed: [01 Variational Autoencoders (VAE · ELBO)](../01-Variational-Autoencoders-VAE-ELBO/01-Variational-Autoencoders-VAE-ELBO.md) — a diffusion model is a many-step hierarchical VAE with a *fixed* encoder; the ELBO, the reparameterization trick, and the Gaussian KL all come from here.
- The KL in every loss term: [01 Foundations · 23 Cross-Entropy and KL Divergence](../../01.%20Foundations/23-Cross-Entropy-and-KL-Divergence/23-Cross-Entropy-and-KL-Divergence.md) — where the two-Gaussian KL (each per-step L_{t-1}) is derived; diffusion specializes it to two Gaussians of equal variance.
- The same ELBO, another view: [04 Unsupervised Learning · 04 Gaussian Mixture Models & EM](../../04.%20Unsupervised_Learning/04-Gaussian-Mixture-Models-and-EM/04-Gaussian-Mixture-Models-and-EM.md) — the variational lower bound and its KL-gap identity that the diffusion ELBO extends over a T-step chain.
- The gradient engine: [05 Deep Learning · 02 Backpropagation and Computational Graphs](../../05.%20Deep_Learning/02-Backpropagation-and-Computational-Graphs/02-Backpropagation-and-Computational-Graphs.md) — the reparameterized forward (x_t = √ᾱ_t x_0 + √(1−ᾱ_t) ε) exists so ordinary backprop can train the denoiser.
- Where it goes next: [06 Score-Based & SDE Diffusion](../06-Score-Based-and-SDE-Diffusion/06-Score-Based-and-SDE-Diffusion.md) — predicting noise is learning the score (ε_θ ≈ −√(1−ᾱ_t) ∇log q_t); the continuous-time SDE picture that unifies DDPM and score matching.
- Where it lives in the real world: [07 Latent Diffusion / Stable Diffusion](../07-Latent-Diffusion-Stable-Diffusion/07-Latent-Diffusion-Stable-Diffusion.md) — run this exact DDPM in a pretrained VAE's latent space; the engine of Stable Diffusion.
- Steering it: [04 Conditional Generation & Classifier-Free Guidance](../04-Conditional-Generation-and-Classifier-Free-Guidance/04-Conditional-Generation-and-Classifier-Free-Guidance.md) — condition the denoiser on text and steer sampling; how text-to-image works on top of the DDPM loop.
- Fixing the slow sampling: [13 Sampling & Guidance Techniques](../13-Sampling-and-Guidance-Techniques/13-Sampling-and-Guidance-Techniques.md) — DDIM and fast samplers that cut the T-step chain to tens of steps.
- The family contrast: [02 GANs and DCGAN](../02-GANs-and-DCGAN/02-GANs-and-DCGAN.md) — adversarial, fast to sample, unstable; diffusion is likelihood-based, stable, sharp, but slow to sample.
