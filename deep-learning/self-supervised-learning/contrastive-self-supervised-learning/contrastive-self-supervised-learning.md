...
- **k-NN evaluation.** Even simpler — no training at all. Embed the labeled set, classify each test point by a **k-nearest-neighbors** vote in embedding space ([k-NN](../../../core-machine-learning/supervised-learning/classification/k-nearest-neighbors/k-nearest-neighbors.md)). A strong k-NN accuracy proves the geometry itself is meaningful: same-class points are genuinely neighbors.
