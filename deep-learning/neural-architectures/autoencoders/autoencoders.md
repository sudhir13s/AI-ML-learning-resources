...
> **Tip:** the "data-specific codec" framing also explains a key failure mode: an autoencoder trained on cats reconstructs cats beautifully and dogs *poorly* — it never learned dog structure. That very weakness is the engine of **[anomaly detection](../../../core-machine-learning/unsupervised-learning/anomaly-detection/anomaly-outlier-detection/anomaly-outlier-detection.md)**: train on normal data, and anything the autoencoder reconstructs *badly* (high reconstruction error) is, by definition, unlike the training distribution — i.e. an anomaly. We return to this.
...
- **Generative modeling.** Make the latent **probabilistic** (the VAE) and you can *sample* new data by drawing a code from the prior and decoding it. This is the conceptual root of **[latent diffusion](../../../modalities-and-generative-models/diffusion-models/latent-diffusion-stable-diffusion/latent-diffusion-stable-diffusion.md)** — Stable Diffusion runs diffusion inside a VAE's latent space because it's far cheaper than pixel space.
...
> **Gotcha:** crank $\beta$ (or KL) too high and you get **posterior collapse** — the encoder gives up and outputs the prior $\mathcal{N}(0,I)$ for *every* input ($\mathrm{KL}\to 0$), so $\mathbf{z}$ carries no information about $\mathbf{x}$ and the decoder ignores it. The KL term won; reconstruction lost entirely. Mitigations (KL annealing, free bits) are covered on the [GenAI VAE page](../../../modalities-and-generative-models/generative-models/variational-autoencoders-vae-elbo/variational-autoencoders-vae-elbo.md).
...
- **[Anomaly / outlier detection](../../../core-machine-learning/unsupervised-learning/anomaly-detection/anomaly-outlier-detection/anomaly-outlier-detection.md).** Train on normal data only; at test time, **reconstruction error is the anomaly score** — points the AE rebuilds poorly are unlike the training distribution. A workhorse for fraud, fault detection, and intrusion detection.
