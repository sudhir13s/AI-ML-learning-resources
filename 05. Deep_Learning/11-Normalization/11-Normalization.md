...
- **[Weight initialization](../05-Weight-Initialization/05-Weight-Initialization.md)** sets the activation/gradient variance *at step 0* (He/Kaiming for ReLU, Xavier/Glorot for tanh). It's a one-time choice that gives training a sane starting scale.
...
- **[Residual connections](../18-Residual-Skip-Connections/18-Residual-Skip-Connections.md)** add an identity shortcut so gradients have a multiplier-free path around the depth, directly fighting [vanishing gradients](../06-Vanishing-Exploding-Gradients/06-Vanishing-Exploding-Gradients.md).
