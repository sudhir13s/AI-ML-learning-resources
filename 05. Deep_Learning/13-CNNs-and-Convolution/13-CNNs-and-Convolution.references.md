...
5. **Make it concrete** — run this chapter's [notebook](code/13-CNNs-and-Convolution.ipynb) (from-scratch conv forward *and* backward, verified), then work through [d2l Ch. 7 (CNNs)](https://d2l.ai/chapter_convolutional-neural-networks/index.html). *Implementing conv/pool layers cements the dimensions and gradients.*
...
- The backward pass a conv plugs into: [02 Backpropagation & Computational Graphs](../02-Backpropagation-and-Computational-Graphs/02-Backpropagation-and-Computational-Graphs.md) — a conv layer's backward pass *is* backprop; all three of its gradients are themselves convolutions (the matmul VJP, specialized to a weight-shared sparse matrix)
- The dense layer a conv replaces: [01 Perceptron & MLP](../01-Perceptron-and-MLP/01-Perceptron-and-MLP.md)
