...
- explain **bidirectional**, **stacked**, and **seq2seq** RNNs, **teacher forcing / exposure bias**, and the **fixed-context bottleneck** that motivated [attention](15-Attention-Mechanism.md);
...
2. **Depth equals sequence length.** A 100-token sequence is a 100-layer-deep tied network. Everything you know about deep nets being hard to train — [vanishing/exploding gradients](06-Vanishing-Exploding-Gradients.md) — applies, *amplified*, because the same matrix is reused at every "layer."
...
For two decades, "model a sequence" essentially meant "use a gated RNN." LSTMs powered Google's machine translation, speech recognition (the acoustic models behind Siri and Google Voice), handwriting recognition, and image captioning. Even now that transformers dominate NLP, the *concepts* here are load-bearing interview material and recur everywhere: the **additive-memory / gradient-highway** idea reappears in [residual connections](18-Residual-Skip-Connections.md), [normalization](11-Normalization.md), and modern **state-space models** (Mamba). And RNNs' **$O(1)$-per-step state** keeps them genuinely competitive for *streaming*, *on-device*, and *very-long-sequence* settings where a transformer's growing [KV cache](../../09.%20LLMs/concepts/05-KV-Cache.md) is too expensive. Understanding *why* RNNs forget — and *how* gating fixes it — is understanding a recurring pattern of deep learning, not a museum piece.
...
> **Note:** teacher forcing is also *why a [KV cache](../../09.%20LLMs/concepts/05-KV-Cache.md) isn't needed in training but is in generation* — training feeds the whole known target at once; generation feeds tokens back one at a time. The RNN-era distinction carries straight into modern LLMs.
...
- **Streaming / low-latency / on-device.** An RNN keeps an **$O(1)$-per-step** state; a transformer's [KV cache](../../09.%20LLMs/concepts/05-KV-Cache.md) grows with context. For real-time speech, keyword spotting, and edge inference, that constant-memory recurrence is a real advantage.
...
| **Memory per step at inference** | **$O(1)$** state — cheap streaming | $O(n)$ [KV cache](../../09.%20LLMs/concepts/05-KV-Cache.md), grows with context |
