...
**Vision Transformers (ViT)** make the opposite bet. A ViT chops the image into patches, embeds each as a token, and runs plain self-[attention](15-Attention-Mechanism.md) — which is **permutation-equivariant** and global from layer one. It has *almost no* built-in spatial prior (just the positional encodings). The consequence is a clean trade:
