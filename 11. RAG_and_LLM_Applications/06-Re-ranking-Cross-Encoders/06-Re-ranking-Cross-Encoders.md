...
With one gold, $\text{IDCG} = 1/\log_2(1+1) = 1$, so nDCG@k equals the gold's own discounted gain $1/\log_2(\text{rank}+1)$: **1.0 at rank 1 ($1/\log_2 2$), 0.631 at rank 2 ($1/\log_2 3$), 0.431 at rank 4 ($1/\log_2 5$), and 0.0 once the gold falls past rank $k$.** Note the discount uses $\text{rank}+1$, so rank 4 gives $1/\log_2(4{+}1)=1/\log_2 5$, *not* $1/\log_2 4$. The notebook prints this discount table (Step 5) so the metric isn't a black box.
...
(nDCG@5 = nDCG@10 = 0.431 for the bi-encoder because the gold sits at rank 4, inside both windows but discounted to $1/\log_2(4{+}1)=1/\log_2 5 = 0.431$.) That is the entire value of re-ranking, in two numbers a reader can reproduce.
