...
![Each lens misses one query type; hybrid catches both. Bars show the rank the correct passage receives under dense, sparse (BM25), and the two fusions, for both probes (rank 1 at top — higher is better). Dense ranks the exact-code gold #2; BM25 misses the paraphrase gold entirely; the weighted hybrid puts both at #1. Generated by `code/make_figures_05.py`.](../images/rag05_lens_miss_catch.png)
...
- **"What if they agree?"** Then fusion reinforces it: a page both specialists rank highly rockets to #1 (this is exactly why a doc that is *strong in both* lists can beat one that is *#1 in only one* — we'll see RRF do this in code).
...
![Incomparable scales: each dot is one passage's score on the exact-code query. Cosine (top row) stays within [0, 0.83]; BM25 (bottom) spreads out to ~5. The BM25 magnitudes are several times larger, so a naive sum is decided by BM25 alone — which is why you must normalize each lens onto a common scale before combining. Generated by `code/make_figures_05.py`.](../images/rag05_scale_mismatch.png)
...
At $\alpha=0.6$ **both** golds rank #1 — MRR and recall@3 both hit **1.0**, strictly beating dense alone (MRR 0.75) and sparse alone (recall 0.5). Note $\alpha=0.5$ is *not* automatically best — here it ties pure sparse — which is the whole reason you tune $\alpha$ per corpus.
...
![RRF mechanics. Left: the weight `1/(k+rank)` by rank for `k=10/60/200` — being #1 is worth far more than #10, and a larger `k` flattens the curve so deep ranks still contribute. Right: a worked fusion of two ranked lists — a doc ranked #2/#1 (A) overtakes one ranked #1/#2 (C), because being strong in *both* lists beats being strong in only one. Illustrative ranks. Generated by `code/make_figures_05.py`.](../images/rag05_rrf_heatmap.png)
