...
![Chunk size vs recall (does some chunk hold the whole answer?) and precision (the answer's share of the chunk). Recall is high but collapses to 0 at size=140, where a boundary splits the fact; precision decays monotonically as chunks grow larger and the fact becomes a smaller fraction of the chunk. There is no single size that maximizes both — the recall/precision tension. Generated by `code/make_figures_02.py`.](../images/rag02_size_recall_precision.png)
