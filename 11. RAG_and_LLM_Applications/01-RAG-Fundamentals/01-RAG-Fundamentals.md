...
![Cosine similarity of all eight corpus passages to the query "When was the Helios-7 satellite launched?", sorted best-first, with the top-3 retrieval cutoff. The answering passage doc[0] scores 0.64 (0.638 to three decimals — the same value the code prints) — far above the rest — so it is retrieved and fed to the generator; the unrelated passages (chessboard, water, Eiffel Tower) score near zero and are left out. Generated by `code/make_figures_01.py`.](../images/rag01_similarity_bars.png)
...
![Retrieval recall@k climbing as k grows, across six paraphrased probe questions (listed in the notebook, Step 7) whose wording barely overlaps their answering passage. With a lexical embedder recall@1 is only 0.50 — half the answers are missed on the first try — rising to 1.00 by k=8. (k=8 retrieves the *entire* 8-passage corpus, so recall is trivially 1.0 at k=N — the informative part of this curve is the left end, where retrieval has to actually choose.) That's the recall/noise knob: a bigger k retrieves the answer more often but stuffs more irrelevant text into the prompt. Better embeddings and re-ranking lift the left end of this curve. Generated by `code/make_figures_01.py`.](../images/rag01_recall_vs_k.png)
