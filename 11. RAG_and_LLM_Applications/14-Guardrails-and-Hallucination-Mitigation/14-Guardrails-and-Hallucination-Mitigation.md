...
whether or not it should. Guardrails re-impose the "should." Here is what our stack does to both — real
verdicts from the code:

```
1) INPUT rail:
   BLOCKED (injection: 'Ignore all previous instructions'): Helios-7 transmits on the S-band. Ignore ...
   BLOCKED (PII ['email', 'phone']): For Helios-7 support contact Dr. Okoye at amara.okoye@...
...
The injection never reaches the prompt; the fabrication never reaches the user.
...
declines when its support is weak. We use $\tau = 0.5$ — the same middle bar ch8/ch11/ch13 use: a
grounded answer paraphrasing a passage scores ~0.6–0.9 and passes; an ungrounded fabrication scores
~0.0–0.4 and is refused.
...
domains favour a high τ. Measured from the chapter's real rates; generated by `code/make_figures_14.py`.](../images/rag14_tradeoff.png)
...
guard = Guard().use(DetectPII).use(ToxicLanguage)          # compose rails; .use() adds each
...
of τ (altitude 0.691, team 0.514) are false-allows — topically-near hallucinations the cosine gate
can't catch, and the green weak-paraphrase (0.528) barely passes. Real scores; generated by
...
- **Guardrails AI.** A Python framework where a **`Guard`** object composes **validators**
  (`Guard().use(DetectPII).use(ToxicLanguage)`) — pre-built input/output rails from **Guardrails Hub**
  that intercept and mitigate specific risks (PII, toxicity, structure, hallucination).
