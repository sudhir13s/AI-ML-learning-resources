...
5. **Make it concrete** — run this chapter's [notebook](code/04-Image-Classification.ipynb): a frozen ResNet-18 + linear probe beats a from-scratch CNN by a measured margin, augmentation shrinks the overfitting gap, and every metric is cross-checked against scikit-learn.
...
**In this platform**:
- Concept page (full explanation): [Image Classification](04-Image-Classification.md)
- Runnable code: [the workflow module `image_classification.py`](code/image_classification.py) · [the step-by-step notebook](code/04-Image-Classification.ipynb) — the module runs the real pipeline on CIFAR-10 (frozen ResNet-18 + linear probe vs a from-scratch CNN, an augmentation ablation, and honest evaluation), with every metric cross-checked against scikit-learn; the notebook runs it one measured step at a time.
- The CNN *mechanism* this workflow uses: [05 Deep Learning › CNNs & Convolution](../../05.%20Deep_Learning/13-CNNs-and-Convolution/13-CNNs-and-Convolution.md) — convolution, filters, receptive fields, the LeNet→ResNet lineage (we *use* a CNN here; that page *derives* it).
- The two moves that carry this chapter: [05 Transfer Learning for Vision](../05-Transfer-Learning-for-Vision/05-Transfer-Learning-for-Vision.md) (freeze vs fine-tune, in depth) · [06 Data Augmentation](../06-Data-Augmentation/06-Data-Augmentation.md) (the transform catalogue and theory).
- The loss, derived: [01 Foundations › Cross-Entropy & KL Divergence](../../01.%20Foundations/23-Cross-Entropy-and-KL-Divergence/23-Cross-Entropy-and-KL-Divergence.md) — softmax cross-entropy as KL from the one-hot label, and its gradient.
- Why generalization is the whole game: [00 Basics › Overfitting & Underfitting](../../00.%20Basics/05-Overfitting-and-Underfitting/05-Overfitting-and-Underfitting.md) (the gap augmentation shrinks) · test-set discipline: [02 Data Preprocessing › Data Leakage](../../02.%20Data_Preprocessing/11-Data-Leakage/11-Data-Leakage.md) · when accuracy lies: [02 Data Preprocessing › Imbalanced Data](../../02.%20Data_Preprocessing/12-Imbalanced-Data/12-Imbalanced-Data.md)
- Where classification goes next: [07 Object Detection](../07-Object-Detection/07-Object-Detection.md) (a detector is a classifier over regions) · [10 Detection & Segmentation Metrics](../10-Detection-and-Segmentation-Metrics/10-Detection-and-Segmentation-Metrics.md) (mAP, IoU — per-class metrics done right) · [11 Vision Transformers](../11-Vision-Transformers/11-Vision-Transformers.md) (the post-CNN classification backbone)
